...
class _LoginWithPinScreenState extends State<LoginWithPinScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: AppImageView.backGroundAssetImage(path: verifyOTPImagePath, child: Padding(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                5.height,
                view.pinTextFieldView(pinController, icon: Icons.lock),
                20.height,
                Row(
                  children: [
                  Expanded(child: view.rememberLoginView(isChecked,onChanged: (bool? value) {
                    setState(() {
                      isChecked = !isChecked;
                    });
                  })),
                    TextButton(onPressed: (){
                      forgetPin();
                    }, child: Text(FORGET_PIN,style: AppTextStyle.semiBoldBlack(),))
                  ],
                ),
                30.height,
                ...
                // const ResendButtonView(),
                50.height,
              ],
            ),
          ),
        ),
      ), size: Size(width, height*.50)),
    );
  }

  loginWithPin(BuildContext context)async{
    var mobile = '${_authController.countryCode.value}${_authController.mobileNumber.value}';
